...
resource "yandex_compute_instance" "bastion-host" {
  ...
  network_interface {
    ...
    dns_record {
      fqdn = "chichnikita-gw.srv."
      ttl  = 300
    }
    nat                = true
    security_group_ids = [yandex_vpc_security_group.chichnikita-gw.id]
...
resource "yandex_alb_load_balancer" "alb1" {
  name               = "alb1"
  network_id         = yandex_vpc_network.network1.id
  security_group_ids = [yandex_vpc_security_group.chichnikita-balancer.id]
...
#chichnikita-balancer

resource "yandex_vpc_security_group" "chichnikita-balancer" {
  name       = "chichnikita-balancer"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "chichnikita-private" {
  name       = "chichnikita-private"
  network_id = yandex_vpc_network.network1.id
  ...
  ingress {
    protocol          = "ANY"
    description       = "any"
    security_group_id = yandex_vpc_security_group.chichnikita-gw.id
  }
...
resource "yandex_vpc_security_group" "chichnikita-public" {
  name       = "chichnikita-public"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "chichnikita-gw" {
  name       = "chichnikita-gw"
  network_id = yandex_vpc_network.network1.id
